try:
    channel_descs = json.loads(open(ROOT.DATA + "roberto.json", "r").read())
...
try:
    persistent_ids = [[int(x[0]), int(x[1])] for x in json.loads(open(ROOT.DATA + "persist.json", "r").read())]
except:
    persistent_ids = []
...
async def cmd_setdesc(interaction: discord.Interaction, description: str = "", custom_channel: discord.abc.GuildChannel = None):
    ...
    if channel.type in ID.CHANNEL.TYPE.THREADS:
        channel = channel.parent
    channel_descs[channel.id] = description
    open(ROOT.DATA + "roberto.json", "w+").write(json.dumps(channel_descs, indent = 4))
...
async def cmd_getdesc(interaction: discord.Interaction, custom_channel: discord.abc.GuildChannel = None):
    ...
    elif channel.type in ID.CHANNEL.TYPE.TEXT:
...
async def cmd_guide(interaction: discord.Interaction, persist: bool = False, buttons: bool = True):
    ...
    open(ROOT.DATA + "persist.json", "w+").write(json.dumps(persistent_ids, indent = 4))
    await interaction.response.send_message("Done!", ephemeral = True, delete_after = 5)
...
async def create_guide(interaction: discord.Interaction, category, page: int = 0):
    ...
    for channel in interaction.guild.channels:
        if channel.type not in ID.CHANNEL.TYPE.ALL:
            continue
        if not channel.permissions_for(interaction.user).view_channel:
            continue
        if channel.category == category:
            channels[channel.position + (10000 * (channel.type in ID.CHANNEL.TYPE.VOICE))] = channel
...
async def create_main_guide(interaction: discord.Interaction, page: int = 0):
    ...
    for category in interaction.guild.categories:
        ...
        for channel in category.channels:
            if not channel.permissions_for(interaction.user).view_channel:
                continue
            if channel.position < top_position and channel.type in ID.CHANNEL.TYPE.TEXT:
                top_channel = channel
        ...
        if top_channel.category:
            channels[category.position + (10000 * (top_channel.type in ID.CHANNEL.TYPE.VOICE))] = top_channel
    ...
    for position in sorted(list(channels)):
        ...
        if channel.category.id in channel_descs:
            desc = channel_descs[channel.category.id].replace("\n", "\n> ")
        else:
            desc = ""
        if channel.category:
